...
public:
    ...
    SpillInfo get1stEvent1stSplill(TTree *,Int_t, float);
...
TRACKER::SpillInfo TRACKER::get1stEvent1stSplill(TTree * trackerTree, Int_t startEvent, float cut){
    ...
    for(int k=startEvent; k<trackerTree->GetEntries()-1; k++) {
        ...
        if ( (TrackerBCONextEvent - TrackerBCOThisEvent)*144e-9 < 0.002 ) {
            ...
            spl.Index = k;
        if (1)        cout << "TRACKER: first event's index and time in spill 1: " << k << " : " << TrackerBCOThisEvent << "\n";
            break;
...
TRACKER::SpillInfo TRACKER::getLastEvent1stSplill(TTree * trackerTree,Int_t startEvent){
    ...
    for(int k=startEvent; k<trackerTree->GetEntries(); k++) {
        ...
        if (k == trackerTree->GetEntries()-1 ){
            ...
            spl.Time = TrackerBCOThisEvent;
            spl.Index = k;
        if (1)    cout << "TRACKER: last event's index and time in spill 1:  " <<  k << " : " << TrackerBCOThisEvent << "\n";
            break;
        }
...
TRACKER::SpillInfo TRACKER::get1stEvent2ndSplill(TTree * trackerTree, Int_t startEvent){
    ...
    for(int k=startEvent; k<trackerTree->GetEntries()-1; k++) {
        ...
        if ( (TrackerBCONextEvent - TrackerBCOThisEvent)*144e-9 < 0.002 ) {
            spl.Time = TrackerBCOThisEvent;
            spl.Index = k;
       if (1)     cout << "TRACKER: first event's index and time in spill 2: " <<  k << " : " << TrackerBCOThisEvent << "\n";
            break;
        }
    }
    return  spl;
...
TRACKER::SpillInfo TRACKER::getLastEvent2ndSplill(TTree * trackerTree, Int_t startEvent){
    ...
    for(int k=startEvent; k<trackerTree->GetEntries()-1; k++) {
        ...
        if ( (TrackerBCONextEvent - TrackerBCOThisEvent)*144e-9 > 0.5) {
            ...
            spl.Time = TrackerBCOThisEvent;
            spl.Index = k;
        if (1)    cout << "TRACKER: last event's index and time in spill 2:  " <<  k << " : " << TrackerBCOThisEvent << "\n";
            break;
        }
    }
    return  spl;
...
TRACKER::SpillInfo TRACKER::get1stEvent3rdSplill(TTree * trackerTree, Int_t startEvent){
    ...
    for(int k=startEvent; k<trackerTree->GetEntries()-1; k++) {
        ...
        if ( (TrackerBCONextEvent - TrackerBCOThisEvent)*144e-9 < 0.002 ) {
            ...
            spl.Time = TrackerBCOThisEvent;
            spl.Index = k;
        if (1)    cout << "TRACKER: first event's index and time in spill 3: " <<  k << " : " << TrackerBCOThisEvent << "\n";
            break;
        }
    }
    return  spl;
